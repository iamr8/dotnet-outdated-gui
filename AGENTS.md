...
- Workflows: `build.yml` (test + verify + buildPlugin + artifact), `codeql.yml` (security;
  ...
  (weekly plugin verifier, pinned to the **current** released Rider only — older builds inside the
  since-build range are deliberately not verified; `recommended()` can resolve 404 EAPs),
